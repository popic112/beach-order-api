 ...
 */
 ...
 /**
  * 🟢 Plasare comandă (umbrella_number extras din baza de date)
  * Endpoint: POST /api/place_orders
  */
 router.post("/", async (req, res) => {
     ...
     try {
         // 🔹 1️⃣ Obținem `umbrella_number` pe baza `qr_code`
         const [umbrellaResult] = await connection.execute(
             "SELECT umbrella_number FROM qr_codes WHERE business_id = ? AND qr_code = ?",
             [business_id, qr_code]
         );
 
         if (!umbrellaResult.length) {
             throw new Error(`Nu există nicio umbrelă asociată acestui QR Code.`);
         }
