...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [umbrellaResult] = await connection.execute('SELECT umbrella_number FROM qr_codes WHERE qr_code = ? AND business_id = ?', [
      "1dcd129e-1012-40d9-9d94-591476f0a2c9",
      "1",
    ])
    ...
    for (const item of order_items) {
      const [rows] = await connection.execute('SELECT price, type FROM menu WHERE id = ? AND business_id = ? AND visible = 1', [
        item.menu_item_id,
        business_id,
      ])
